...
if [ "$CHECK" = true ]; then
  ...
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/labseven/ \
                                     --checkfiles CommandLineGeometer.java --singlecomments 5 --multicomments 2 --language Java
  determine_exit_code $?
  # --> GatorGrader CHECK: the correct number of comments in the Java code
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/labseven/ \
                                     --checkfiles GeometricCalculator.java --multicomments 4 --language Java
  determine_exit_code $?
  # --> GatorGrader CHECK: the "new Date()" fragment exists in the code at least once
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/labseven \
                                     --checkfiles CommandLineGeometer.java --fragments "new Date()" --fragmentcounts 1
  determine_exit_code $?
  # --> GatorGrader CHECK: the "new Date()" fragment does not exist in the code at least once
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/labseven \
